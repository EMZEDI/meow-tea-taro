...
def need_critic(config: DictConfig) -> bool:
    ...
    elif config.algorithm.adv_estimator in [AdvantageEstimator.GAE, AdvantageEstimator.BC_GAE]:
        return True
    else:
        warnings.warn(
            "Disabled critic as algorithm.adv_estimator not in [gae, bc_gae]. If it is not intended, please set critic.enable=True",
            stacklevel=2,
        )
        return False
